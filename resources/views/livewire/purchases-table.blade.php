<div class="shadow-xl pl-2 pr-2 pb-4">
    <p class="flex justify-center items-center mb-6 text-xl">List of All Purchased Transaction</p>
        ...
        <tbody>
            @foreach($purchases as $purchase)
                ...
                <tr>
                    ...
                    <td class="border px-4 py-1 dark:border-gray-500">{{ ucfirst($purchase->product_name) }}</td>
                    <td class="border px-4 py-1 dark:border-gray-500">{{ ucfirst($purchase->product_description) }}</td>
                    <td class="border px-4 py-1 dark:border-gray-500">{{ ucfirst($purchase->product_quantity) }}</td>
                    <td class="border px-4 py-1 dark:border-gray-500">{{ $purchase->unit_cost }}</td>
                    {{-- <td class="border px-4 py-1 dark:border-gray-500">{{ $purchase->supplier_id }}</td> --}}
                    <td class="border px-4 py-1 dark:border-gray-500">{{ $purchase->supplier }}</td>
                    <td class="border px-4 py-1 dark:border-gray-500">{{ $purchase->created_at}}</td>
                    ...
                    <td class="flex items-center justify-center border px-4 py-2 gap-6 dark:border-gray-500">
                        ...
                        <a href="{{url('edit-purchase-product/'.$purchase->id)}}" > <button> <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#3F83F8" class="w-6 h-6 mr-2">
                        ...
                        <a href="{{url('delete-product/'.$purchase->id)}}" > 
