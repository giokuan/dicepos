<div class="shadow-xl pl-2 pr-2">
    <p class="flex justify-center items-center mb-6 text-xl">List of All Customers</p>
    <div class="w-full flex flex-col md:flex-row  pb-10">
        <div class="w-full md:w-3/6 mx-1">
            <label class="text-xs ">Search</label>
            <input wire:model.debounce.300ms="search" type="text" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-1 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 shadow-lg"placeholder="Search customer...">
        </div>
    ...
    <table class="table-auto w-full mb-6 overflow-x-auto">
        ...
        <tbody>
            @foreach($customers as $customer)
                ...
                <tr>
                    <td class="border px-4 py-2 dark:border-gray-500" >{{ ucfirst($customer->id) }}</td>
                    <td class="border px-4 py-2 dark:border-gray-500" >{{ ucfirst($customer->customername) }}</td>
                    <td class="border px-4 py-2 dark:border-gray-500">{{ ucfirst($customer->contact) }}</td>
                    <td class="border px-4 py-2 dark:border-gray-500">{{ ucfirst($customer->address) }}</td>
                    ...
                    <td class="flex items-center justify-center border px-4 py-2 dark:border-gray-500">
                        ...
                        <a href="{{url('edit-customer/'.$customer->id)}}" >  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#3F83F8" class="w-6 h-6 mr-2">
