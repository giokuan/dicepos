<div class="shadow-xl pl-2 pr-2">
    <p class="flex justify-center items-center mb-6 text-xl">List of All Purchased Suppliers</p>
    <div class="w-full flex flex-col md:flex-row  pb-10">
        <div class="w-full md:w-3/6 mx-1">
            <label class="text-xs ">Search</label>
            <input wire:model.debounce.300ms="search" type="text" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-1 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 shadow-lg"placeholder="Search supplier...">
        </div>
    ...
    <table class="table-auto w-full mb-6 overflow-x-auto">
        ...
        <tbody>
            @foreach($suppliers as $supplier)
                ...
                <tr>
                    <td class="border px-4 py-2 dark:border-gray-500" >{{ ucfirst($supplier->id) }}</td>
                    <td class="border px-4 py-2 dark:border-gray-500" >{{ ucfirst($supplier->suppliername) }}</td>
                    <td class="border px-4 py-2 dark:border-gray-500">{{ ucfirst($supplier->contact) }}</td>
                    <td class="border px-4 py-2 dark:border-gray-500">{{ ucfirst($supplier->address) }}</td>
                    ...
                    <td class="flex items-center justify-center border px-4 py-2 dark:border-gray-500">
                        ...
                        <a href="{{url('edit-supplier/'.$supplier->id)}}" >  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#3F83F8" class="w-6 h-6 mr-2">
                        ...
                        <a href="{{url('delete-supplier/'.$supplier->id)}}" > 
